 ...
 *  1) Fetch CSV from NASA for today's date if the file doesn't exist.
...
async function fetchAndCompareFireData() {
  try {
    ...
    // 3) Create a filename for today's data
    ...
    if (fs.existsSync(todayFilePath)) {
      // If today's file exists, read from it
      const todayDataString = fs.readFileSync(todayFilePath, 'utf-8');
      newData = JSON.parse(todayDataString);
      console.log(`[fetchFires] Read today's data from ${todayFilename}`);
    } else {
      // 4) Fetch the CSV data
      const response = await axios.get(NASA_FIRMS_CSV_URL);
      const csvData = response.data; // This is the raw CSV string

      // 5) Convert CSV to JSON (array of objects)
    ...
    if (fs.existsSync(yesterdayFilePath)) {
      ...
      const makeKey = (obj) => `${obj.latitude},${obj.longitude},${obj.bright_ti4}`;
